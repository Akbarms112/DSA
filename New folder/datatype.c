...
#include<process.h>
#include<string.h>
//the library functions are included to the program.
//a structure has been defined which is used to maintain the details of any book
typedef struct
{
	char book_name[30];    //declaring a structure
	char author[30];
	int b_id;
	int a_id;
	int copy;
...
void main()
{
 ...
 {
 case 1: user(records);  //calling the user function to perform required tasks by the user
	break;
 case 2:  printf(" ENTER THE PASSWORD(IN CAPITAL LETTER) :");
                scanf("%s",str);  //taking the password as input
 if(strcmp(str,"BOOKSHOP")==0)
	{      //matching the password given as input with the stored password                                                                                                                                                                                                                                        {
...
case 0:  exit(0); //termination of program
  }//end of switch case
...
{
	FILE *p,*t,*q,*r;   //declaring file pointer
	char an , b; //variable
	int i,c,m,n=0;
	int a;        //declaration
	static float sum;
  ...
  switch(i)
{
  case 1:  fseek(p,0,SEEK_END);                   //keep the file pointer at the end of file
                an='y';
	while(an=='y')
	{
	...
	an=getche();
	  ...
	  break;
  case 2: rewind(p);                                   //keeps the file pointer at the startinf of file
	...
	printf("NUMBER OF COPIES=%d",records.copy);
        ...
        getch();
   break;
 case 3:  fseek(t,0,SEEK_END);                               //keep the file pointer at the end of file
               an='y';
	...
	scanf("%d",&records.copy);
           ...
           break;
  case 4:rewind(t);                                       //keep the file pointer at the starting of file
...
break;
case 7:  rewind(q);                                  //the pointer is placed at the first position of file
...
getch();
 break;
 case 0:   fclose(p);                          //closing the file BOOK.C
	...
	if(p==NULL)
		{
		 ...
		 if(t==NULL)
		 {
		  ...
		  while(1)
		  {
           ...
           switch(i)
          {
             case 1:  printf(" ENTER THE BOOK NAME(IN CAPITAL LETTER):");
...
getch();
break;
case 2: printf(" ENTER THE AUTHOR NAME(IN CAPITAL LETTER):");
